"""
Test script for the thread-safe NIM generation function - using true in-memory databases
"""
import sys
import os
...
def test_format():
    ...
    # Create truly in-memory database for testing (with :memory: and shared cache)
...
def test_real_world_scenario():
    ...
    # Import models and other requirements
    import models
    
    # Create truly in-memory database for testing
    engine = create_engine('sqlite:///:memory:')
    Base.metadata.create_all(bind=engine)
    ...
    for i in range(1, 6):
        calon_mahasiswa = models.CalonMahasiswa(
            ...
            email=f"student{i}.real@examplereal{i}.com",
            phone="081234567890",
            tanggal_lahir=datetime.now(),
