...
# Import the models and main app
import models
from models import Base, CalonMahasiswa, ProgramStudi, JalurMasukEnum, StatusEnum
from database import get_db

# Import the main app after setting up dependencies to avoid relative import issues
import sys
from pathlib import Path
sys.path.append(str(Path(__file__).parent))

# Create the FastAPI app with absolute imports
...
def test_invalid_phone_format(client, db_session):
    ...
    if response.status_code == 422:  # Pydantic validation error
        assert "phone" in str(response.json())
