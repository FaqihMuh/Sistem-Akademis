...
def import_modules():
    ...
    try:
        ...
        from .database import get_db as _get_db_func, SessionLocal as _SessionLocal
...
app = FastAPI(title="PMB System", description="Sistem Penerimaan Mahasiswa Baru")

# Create tables after all modules are loaded (this avoids circular imports)
...
@app.get("/")
def read_root():
    return {"message": "Welcome to PMB System API"}
...
@app.get("/api/pmb/stats", response_model=Dict[str, int])
def get_pmb_stats(
    db: Session = Depends(get_db)
):
...
@app.post("/calon-mahasiswa/", response_model=schemas.CalonMahasiswaResponse)
def create_calon_mahasiswa(
    calon_mahasiswa: schemas.CalonMahasiswaCreate,
    db: Session = Depends(get_db)
):
    # Check if email already exists
...
@app.post("/program-studi/", response_model=schemas.ProgramStudiResponse)
